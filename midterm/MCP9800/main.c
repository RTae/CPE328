...
int main(void) {
    ...
    while (1) {
        temp_raw = readTempMCP9600();
        temp_cov = temp_raw / 256;
        
        
        dtostrf(temp_cov, 4, 2, buffer);
        strcat(buffer, " C\r\n");
        print(buffer);
        _delay_ms(1000);
    }
